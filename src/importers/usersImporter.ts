...
export async function importUsers(db: TriDB, srcFile: string) {
    ...
    const query = "INSERT INTO tri.users (username, legal_name, country, state, password_hash, passkey_user_id) VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE state = ?, passkey_user_id = ?";
